...
export async function registerRoutes(app) {
    ...
    app.post('/api/users/type', isAuthenticated, async (req, res) => {
        try {
            const userId = req.user.claims.sub;
    ...
    app.get('/api/users/:userId/properties', isAuthenticated, async (req, res) => {
        try {
    ...
    app.post('/api/properties/:id/images', isAuthenticated, async (req, res) => {
        try {
    ...
    app.delete('/api/properties/images/:id', isAuthenticated, async (req, res) => {
        try {
    ...
    app.post('/api/properties/:id/features', isAuthenticated, async (req, res) => {
        try {
            const propertyId = parseInt(req.params.id);
            const userId = req.user.claims.sub;
            // Check if property exists and belongs to the user
    ...
    app.get('/api/properties/:id/messages', isAuthenticated, async (req, res) => {
        try {
            const propertyId = parseInt(req.params.id);
            const userId = req.user.claims.sub;
            // Check if property exists and belongs to the user
    ...
    app.get('/api/conversations/:userId/:propertyId', isAuthenticated, async (req, res) => {
        try {
            const currentUserId = req.user.claims.sub;
    ...
    app.post('/api/offers', isAuthenticated, async (req, res) => {
        try {
            ...
            // Ensure user can't make an offer on their own property
            const property = await storage.getProperty(offerData.propertyId);
            if (!property) {
                return res.status(404).json({ message: "Property not found" });
            }
            if (property.userId === buyerId) {
                return res.status(400).json({ message: "You cannot make an offer on your own property" });
            }
    ...
    app.get('/api/properties/:id/offers', isAuthenticated, async (req, res) => {
        try {
            const propertyId = parseInt(req.params.id);
            const userId = req.user.claims.sub;
            // Check if property exists and belongs to the user
    ...
    app.get('/api/users/:id/offers', isAuthenticated, async (req, res) => {
        try {
            const requestedUserId = req.params.id;
            const currentUserId = req.user.claims.sub;
            // Users can only see their own offers
            if (requestedUserId !== currentUserId && req.user.userType !== 'admin') {
                return res.status(403).json({ message: "Unauthorized" });
            }
    ...
    app.put('/api/offers/:id/status', isAuthenticated, async (req, res) => {
        try {
    ...
    if (stripe) {
        app.post('/api/create-payment-intent', isAuthenticated, async (req, res) => {
            try {
                ...
                // Create payment intent for $999
                const amount = 99900; // $999.00 in cents
        ...
        app.post('/api/premium-listing/verify', isAuthenticated, async (req, res) => {
            try {
                const userId = req.user.claims.sub;
                const { paymentIntentId, propertyId } = req.body;
                if (!paymentIntentId || !propertyId) {
                    return res.status(400).json({ message: "Payment intent ID and property ID are required" });
                }
                ...
                // Set premium status (valid for 30 days)
                const premiumUntil = new Date();
                premiumUntil.setDate(premiumUntil.getDate() + 30);
                const updatedProperty = await storage.setPremiumStatus(parseInt(propertyId), true, premiumUntil);
                // Record the transaction
                await storage.recordPremiumTransaction({
                    userId,
